...
class Harmor(Gen):
    ...
    def __init__(self, sample_rate=16000, name='harmor', n_harmonics=24, sep_amp=False, n_oscs=2, freq_range=WIDE_RANGE):
...
class HarmorDecoupled(Gen):
    """
    Oscillator pitch is decoupled
    (osc1 can be higher than osc2 in pitch)
    """
    ...
    def __init__(self, sample_rate=16000, name='harmor', n_harmonics=24, sep_amp=False, n_oscs=2, freq_range=WIDE_RANGE):
        ...
        self.param_desc = {
            'amplitudes':       {'size': n_amps, 'range': (0, 1),    'type': 'sigmoid'},
            'osc_mix':          {'size': self.n_oscs, 'range': (0, 1),    'type': 'sigmoid'}, 
            'f0_hz':            {'size': self.n_oscs, 'range': freq_range, 'type': 'freq_sigmoid'},
            'cutoff':           {'size': 1, 'range': (30.0, self.sample_rate/2), 'type': 'freq_sigmoid'},
            'q':                {'size': 1, 'range': (0.1, 2.0), 'type': 'sigmoid'}
            }
        

    def forward(self, amplitudes, osc_mix, f0_hz, cutoff, q, n_samples):
        """Synthesize audio with additive synthesizer from controls.
