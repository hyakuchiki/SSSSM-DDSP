...
class FX(Synthesizer):
    def __init__(self, dag, name='fx', fixed_params={}, static_params=[]):
        super().__init__(dag, name, fixed_params, static_params)
        self.ext_param_size -= 1
        self.ext_param_sizes.pop('input_audio')
    
    def forward(self, dag_inputs, input_audio, n_samples):
        """runs input through DAG of processors

        Args:
            dag_inputs (dict): ex. {'INPUT_KEY':Tensor}
            input_audio: (batch, n_samples)
        Returns:
            dict: Final output of processor
        """
        outputs = dag_inputs
        outputs['input_audio'] = input_audio
        for node in self.dag:
            processor, connections = node
            # fixed params are not in 0~1 and do not need to be scaled
            scaled = [k for k in connections if connections[k] in self.fixed_param_names]
